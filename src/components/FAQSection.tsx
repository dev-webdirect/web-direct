...
export const FAQSection = () => {
  ...
  return (
    <section className="py-20 px-6 lg:px-12 bg-white dark:bg-[#0f0a1f] transition-colors duration-300">
    <div className="max-w-5xl mx-auto">
      <div className="grid lg:grid-cols-2 gap-16 lg:gap-20">
        <div>
          <h2 className="font-bold text-4xl sm:text-5xl lg:text-6xl text-[#30294e] dark:text-white mb-8 leading-tight tracking-tight">
        ...
        <div className="space-y-5">
          {faqs.map((faq, i) => <div key={i} className="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden bg-white dark:bg-[#1a1227] hover:shadow-xl transition-all">
              <button className="w-full p-8 text-left flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
              ...
              opacity: 0
                    ...
                    <div className="px-8 pb-8 text-gray-600 dark:text-gray-300 leading-relaxed text-base">{faq.a}</div>
                  </motion.div>}
              </AnimatePresence>
            </div>)}
        </div>
