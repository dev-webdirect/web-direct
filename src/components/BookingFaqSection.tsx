...
const FaqAccordionItem = ({
  ...
  return (
    <div className="border border-gray-800 rounded-2xl overflow-hidden bg-[#1a1227] hover:shadow-2xl transition-shadow duration-300">
      <button
        onClick={onToggle}
        className="w-full p-4 sm:p-5 md:p-6 lg:p-8 text-left flex justify-between items-start lg:items-center hover:bg-white/[0.02] transition-colors gap-3 sm:gap-4"
      >
        ...
        <div className={cn(
          "mt-1 shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300",
          isOpen ? "bg-[#6a49ff] text-white" : "bg-gray-800 text-gray-500"
        )}>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-4 sm:px-6 lg:px-8 pb-6 sm:pb-8 text-gray-400 leading-relaxed text-base border-t border-white/5 pt-4 sm:pt-6">
              {answer}
            </div>
          </motion.div>
        )}
...
const CategoryTab = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "relative flex items-center gap-3 px-6 py-3 rounded-xl text-sm font-medium transition-all duration-300 whitespace-nowrap w-full justify-center",
        isActive
          ? "bg-[#6a49ff] text-white shadow-lg shadow-[#6a49ff]/20"
          : "bg-[#1a1227] text-gray-400 hover:bg-[#251b36] hover:text-white border border-gray-800"
      )}
    >
...
export const BookingFaqSection = () => {
  ...
  const direction = activeCategory > prevCategory ? 1 : -1;
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full">
        ...
        <div className="text-center mb-10 sm:mb-12 md:mb-16 space-y-4 sm:space-y-6">
          ...
          >
            ...
            <span className="relative inline-block italic font-large text-transparent bg-clip-text bg-linear-to-r from-[#6a49ff] to-[#a78bfa] font-serif">
              {t('heading.highlight')}
            </span>
          </motion.h2>
        ...
        <motion.div
          ...
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 mb-8 sm:mb-10 md:mb-12 max-w-4xl mx-auto"
        >
        ...
        {/* FAQ Accordion — smooth directional slide */}
        <div className="max-w-4xl mx-auto mb-12 sm:mb-16 md:mb-20 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            <h3 className="text-white font-bold text-3xl mb-3">{t('support.title')}</h3>
            <p className="text-gray-400 text-lg">{t('support.description')}</p>
          </motion.div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10 md:mb-12">
            {[
              { icon: Mail, delay: 0.1 },
              { icon: MessageCircle, delay: 0.2 },
          ...
          >
            <div className="absolute inset-0 rounded-2xl bg-linear-to-br from-[#6a49ff]/10 to-[#41AE96]/10 opacity-50 pointer-events-none" />

            <motion.div
              className="absolute -top-10 -right-10 w-40 h-40 bg-[#6a49ff]/20 rounded-full blur-3xl pointer-events-none"
              animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.3, 0.2] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.div
              className="absolute -bottom-10 -left-10 w-40 h-40 bg-[#41AE96]/20 rounded-full blur-3xl pointer-events-none"
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-4 sm:gap-6 md:gap-8">
              ...
              <motion.button
                onClick={goToBooking}
                whileHover={{ scale: 1.05, boxShadow: "0 20px 25px -5px rgba(106, 73, 255, 0.3), 0 10px 10px -5px rgba(106, 73, 255, 0.2)" }}
                whileTap={{ scale: 0.95 }}
                className="flex items-center gap-3 bg-linear-to-r from-[#6a49ff] to-[#5839e6] text-white px-10 py-5 rounded-full font-semibold text-lg transition-all shadow-xl shadow-[#6a49ff]/20 hover:shadow-[#6a49ff]/40 group whitespace-nowrap"
              >
