/**
 * Fluid simulation - can be used as a module (initFluidSim) or as a script tag.
 * When used as module: import { initFluidSim } from './fluid-sim' and call with config including targetEl.
 */
export function initFluidSim(globalConfig) {
    "use strict";

    if (!globalConfig || !globalConfig.targetEl) {
        console.error('fluid-sim: targetEl is required when using initFluidSim');
        return () => {};
    }
    ...
    const parentElement = globalConfig.targetEl;
    ...
    // let config =  {
        ...
        BLOOM_THRESHOLD: 1,
        BLOOM_SOFT_KNEE: 0.25,
        SUNRAYS: true,
        SUNRAYS_RESOLUTION: "512",
        SUNRAYS_WEIGHT: 0.50,
        CUSTOM_COLOR: false,
        COLOR_RANGE: ["#be4c15", "#be4c15"],
    };
...
`
    ...
    const getMousePos = (e) => {
        ...
        const x = target === parentElement ? e.offsetX : (e.clientX - rect.left);
        const y = target === parentElement ? e.offsetY : (e.clientY - rect.top);
        return { posX: scaleByPixelRatio(x), posY: scaleByPixelRatio(y) };
    };
    ...
    const handleMouseUp = () => updatePointerUpData(pointers[0]);

    const mouseEventTarget = globalConfig.mouseEventTarget || parentElement;
