...
export async function GET(req: NextRequest) {
  ...
  if (startParam && endParam) {
    ...
    startMs = Math.max(reqStart.getTime(), minStart.getTime());
    endMs   = Math.min(reqEnd.getTime(), maxEnd.getTime());
  } else {
    startMs = minStart.getTime();
    endMs   = Math.min(minStart.getTime() + 7 * 24 * 60 * 60 * 1000, maxEnd.getTime());
  }
  ...
  try {
    ...
    for (const [, value] of Object.entries(data)) {
      if (!value || typeof value !== 'object' || !Array.isArray((value as Record<string, unknown>).slots)) continue;

      for (const slotIso of (value as { slots: string[] }).slots) {
        if (new Date(slotIso).getTime() >= cutoff) {
          collection.push({ start_time: slotIso });
        }
      }
    }
